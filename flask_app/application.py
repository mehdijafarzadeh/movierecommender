...
from movierecommender.utils import movies, ratings, model, lookup_movie
from movierecommender.recommender import recommend_random, recommend_nmf
...
def recommend():
    ...
    # extracts the movie titles from the requests.arg
    movie_titles = []
    for query in queries:
        matches = lookup_movie(query, movies['title'])
    ...
    recommendations = recommend_nmf(movie_titles, rating, k=5)

    return render_template('recommend.html', titles=recommendations)
